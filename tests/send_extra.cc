...
int main() {
  try {
...

    {
      ...
      test.execute(
        ExpectSegment {}.with_payload_size(3).with_data("abc").with_seqno(isn + 1));
...

    {
      ...
      test.execute(WriteBytes("def"));
      test.execute(
        ExpectSegment {}.with_payload_size(3).with_data("def").with_seqno(isn + 4));
...

    {
      ...
      test.execute(
        ExpectSegment {}.with_payload_size(3).with_data("ghi").with_seqno(isn + 7));
      test.execute(AckReceived { WrappingInt32 { isn + 1 } }.with_win(1000));
      test.execute(Tick { 6 });
      test.execute(
        ExpectSegment {}.with_payload_size(3).with_data("abc").with_seqno(isn + 1));
...

    {
      ...
      TCPSenderTestHarness test { "Retransmit a FIN-containing segment same as any other",
                                  cfg };
      test.execute(
        ExpectSegment {}.with_no_flags().with_syn(true).with_payload_size(0).with_seqno(
          isn));
...

    {
      ...
      TCPSenderTestHarness test { "Retransmit a FIN-only segment same as any other",
                                  cfg };
      test.execute(
        ExpectSegment {}.with_no_flags().with_syn(true).with_payload_size(0).with_seqno(
          isn));
      ...
      test.execute(Close {});
      test.execute(
        ExpectSegment {}.with_payload_size(0).with_seqno(isn + 4).with_fin(true));
      test.execute(Tick { rto - 1 });
      test.execute(ExpectNoSegment {});
...

    {
      ...
      TCPSenderTestHarness test { "When filling window, treat a '0' window size as equal "
                                  "to '1' but don't back off RTO",
                                  cfg };
      test.execute(
        ExpectSegment {}.with_no_flags().with_syn(true).with_payload_size(0).with_seqno(
          isn));
      ...
      test.execute(
        ExpectSegment {}.with_payload_size(0).with_data("").with_seqno(isn + 4).with_fin(
          true));
...

    {
      ...
      test.execute(
        ExpectSegment {}.with_payload_size(7).with_data("abcdefg").with_seqno(isn + 1));
